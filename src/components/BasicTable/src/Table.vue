...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const getBindValues = computed(() => {
            ...
            // 列表插入序列
            if (getPageColumns.value[0].title == "序列") {
                let render = (_: any, index: number) =>
                    `${
                        index +
                        1 +
                        (props.pageOption.currentPage - 1) *
                            props.pageOption.pageSize
                    }`;
                getPageColumns.value[0].render = render;
            }
            const maxHeight = tableData.length
