    ...
    <n-form v-bind="getBindValue" :model="formModel" ref="formElRef">
        <n-grid v-bind="getGrid">
            <n-gi v-bind="schema.giProps" v-for="schema in getSchema" :key="schema.field">
                <n-form-item :label="schema.label" :path="schema.field" v-bind="schema.formItemProps">
                    <!--标签名右侧温馨提示-->
                    ...
                    <template v-else-if="schema.component === 'NUpload'">
                        <n-upload v-bind="getComponentProps(schema)" v-model:file-list="schema.componentProps.fileList">
                            <template v-if="schema.componentProps.listType != 'image-card'"> <n-button>上传文件</n-button></template>
                        </n-upload>
                    </template>
                    <!--NTreeSelect-->
                    <template v-else-if="schema.component === 'NTreeSelect'">
                        <n-tree-select v-bind="getComponentProps(schema)" v-model:value="formModel[schema.field]"> </n-tree-select>
                    </template>
                    <!--textareaText富文本框-->
                    <template v-else-if="schema.component === 'TextareaText'">
                        <textareaText v-model="formModel[schema.field]"></textareaText>
                    </template>
                    <!--动态渲染表单组件-->
                    <component
                        v-else
...
import { deepMerge } from '@/utils/common/index';

import textareaText from '@/components/textareaText/index.vue';

export default defineComponent({
    name: 'BasicUpload',
    components: { DownOutlined, UpOutlined, QuestionCircleOutlined, textareaText },
    props: {
        ...basicProps,
    },
    ...
    setup(props, { emit, attrs }) {
        ...
        const { handleSubmit, validate, resetFields, getFieldsValue, clearValidate, setFieldsValue, updateSchema } = useFormEvents({
            emit,
            getProps,
            formModel,
            getSchema,
