<template>
  ...
  import { getAlarmMessageList } from "@/api/alarmInfo";
  import { getEnumType } from "@/api/interfaceLog";
  import type { PaginationType } from "@/components/pagination/index";
  import { AppstoreAddOutlined } from "@vicons/antd";
  import moment from "moment";
  const searchData = ref<AlarmMessage.AlarmMessagedata>({});
  const tableData = ref<AlarmMessage.AlarmMessageList[]>([]);
  ...
  function handleSubmit(values: Recordable) {
    ...
    data.selectData.alarmType = values.alarmType;
  ...
  const getprotocolData = async () => {
    const RoteData = (await getEnumType("JT808Alarm")).Data;
