    ...
    <div id="Map" class="Map">
        ...
        <div ref="popupRef">
            ...
            <n-space justify="space-around" size="large">
                <div class="popup_btn" @click="openDialog(infoData?.terminalNo as string)">
    ...
    data.forEach(device => {
        if(layersData.value.editCarData[device.terminalNo] == undefined) {
            if(device.lat && device.lng) {
                ...
                layersData.value.editCarData[device.terminalNo] = L.marker(latLng, {
                    icon,
                })
                .bindPopup(initPopup(device))
                .on('click', () => changeCarPopup([layersData.value.popupData[device.terminalNo]]))
...
const removeCar = (terminalNo: string[]) => {
    terminalNo.forEach(item => {
        if(layersData.value.editCarData[item]) {
            layers.value.carLayer.removeLayer(layersData.value.editCarData[item]);
            ...
            setTimeout(() => {
                if(layersData.value.editCarData[data.TN]) {
                    ...
                    .slideTo([data.Lat, data.Lng], {
                        duration: Number(mapParamData.value.duration) * 1000,
                    });
                    if(infoData.value && infoData.value?.terminalNo == data.TN) {
