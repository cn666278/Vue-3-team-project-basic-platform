<template>
  <n-layout has-sider>
    <n-layout-sider collapse-mode="transform" :collapsed-width="0" :width="320" show-trigger="bar" content-style="padding: 24px;" bordered>
...
const businessGroupId = ref<string>("");
// 定义树形参数
const treeObj = ref();
// 新增弹窗显示
...
const getTableData = async () => {
  if (data.selectData) {
    data.selectData.businessGroupId = businessGroupId.value;
...
/**树形数据格式化 */
const treeDataRecursive = (data: BusinessGroup.BusinessGroupList[]) => {
  let treeData = data;
...
const zTreeOnNodeClick = (event: Event, treeId: string, treeNode: any) => {
  ...
  businessGroupId.value = treeNode.id;
  getTableData();
};
</script>
<style lang="scss">
